...
const Boat = require('../service/BoatService'); // Importation du service des bateaux
const User = require('../service/UserService.js')

// Fonction pour créer un voyage en bateau 
module.exports.createBoatTrip = async function createBoatTrip(req, res) {
    try {
        ...
        if (!boat_id) {
            return utils.writeJson(res, {
                success: false,
                errorCode: "MISSING_BOAT_ID",
                message: "L'identifiant du bateau est requis pour créer une sortie en mer."
            }, 400);
        }

        // Vérifier si le bateau existe
        const boat = await Boat.getBoatById(boat_id);
        if (!boat) {
            console.warn(`Tentative de création d'un BoatTrip avec un boat_id inexistant: ${boat_id}`);
            return utils.writeJson(res, {
                success: false,
                errorCode: "BOAT_NOT_FOUND",
                message: "Le bateau spécifié n'existe pas."
            }, 404);
        }
        ...
        // Création du voyage en bateau
        const createdBoatTrip = await BoatTrip.createBoatTrip({
            title, practicalInformation, type, priceType, startDate, endDate, passengersNumber, price, user_id, boat_id
        });

        return utils.writeJson(res, createdBoatTrip, 201);
        ...
        return utils.writeJson(res, {
            ...
            message: "Erreur interne du serveur, mais l'application continue de fonctionner."
        }, 500);
    }
};
