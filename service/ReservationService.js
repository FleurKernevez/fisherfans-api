...
exports.createReservation = function (boatTrip_id, choosenDate, seatsBooked, totalPrice, userId) {
  return new Promise((resolve, reject) => {
    ...
    database.get(getBoatTripCapacityQuery, [boatTrip_id], (err, boatTrip) => {
      ...
      const maxCapacity = boatTrip.passengersNumber;
      ...
      database.get(getTotalReservedSeatsQuery, [boatTrip_id], (err, result) => {
        ...
        if (totalSeatsReserved + seatsBooked > maxCapacity) {
          console.warn(`Capacité maximale dépassée pour BoatTrip ${boatTrip_id}`);
          return reject({ message: "Le nombre de places réservées dépasse la capacité du bateau.", code: "LIMIT_EXCEEDED" });
        }
